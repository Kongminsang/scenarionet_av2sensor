...
class ScenarioDescriptionMapless(dict): # kong_todo :ScenarioDescription_mapless 구현
    """
    ...
    FIRST_LEVEL_KEYS = {TRACKS, VERSION, ID, LENGTH, METADATA}
    ...
    def sanity_check(cls, scenario_dict, check_self_type=False, valid_check=False):
        ...
        if check_self_type:
            ...
            assert not isinstance(scenario_dict, ScenarioDescriptionMapless)
        ...
        for obj_id, obj_state in scenario_dict[cls.TRACKS].items():
            ...
            assert ScenarioDescriptionMapless.HEADING in obj_state[ScenarioDescriptionMapless.STATE
                                                            ], "heading is required for an object"
            assert ScenarioDescriptionMapless.POSITION in obj_state[ScenarioDescriptionMapless.STATE
    ...
    def _check_map_features(cls, map_feature):
        ...
        for id, feature in map_feature.items():
            if MetaDriveType.is_lane(feature[ScenarioDescriptionMapless.TYPE]):
                assert ScenarioDescriptionMapless.POLYLINE in feature, "No lane center line in map feature"
                assert isinstance(
                    feature[ScenarioDescriptionMapless.POLYLINE], (np.ndarray, list, tuple)
    ...
    def get_object_summary(object_dict, object_id: str):
        ...
        return {
            ScenarioDescriptionMapless.SUMMARY.TYPE: object_type,
            ScenarioDescriptionMapless.SUMMARY.OBJECT_ID: str(object_id),
            ScenarioDescriptionMapless.SUMMARY.TRACK_LENGTH: int(len(track)),
            ScenarioDescriptionMapless.SUMMARY.MOVING_DIST: float(distance),
            ScenarioDescriptionMapless.SUMMARY.VALID_LENGTH: int(valid_length),
            ScenarioDescriptionMapless.SUMMARY.CONTINUOUS_VALID_LENGTH: int(continuous_valid_length)
        }
    ...
    def get_export_file_name(dataset: str, dataset_version: str, scenario_name: str):
        ...
        return "sd_mapless_{}_{}_{}.pkl".format(dataset, dataset_version, scenario_name)
    ...
    def _calculate_num_moving_objects(scenario):
        ...
        # moving object
        number_summary_dict = {
            ScenarioDescriptionMapless.SUMMARY.NUM_MOVING_OBJECTS: 0,
            ScenarioDescriptionMapless.SUMMARY.NUM_MOVING_OBJECTS_EACH_TYPE: defaultdict(int)
        }
        for v in scenario[ScenarioDescriptionMapless.METADATA][ScenarioDescriptionMapless.SUMMARY.OBJECT_SUMMARY].values():

            # Fix a tiny compatibility issue
            if ScenarioDescriptionMapless.SUMMARY.MOVING_DIST not in v:
                v[ScenarioDescriptionMapless.SUMMARY.MOVING_DIST] = v['distance']

            if v[ScenarioDescriptionMapless.SUMMARY.MOVING_DIST] > 1:
                number_summary_dict[ScenarioDescriptionMapless.SUMMARY.NUM_MOVING_OBJECTS] += 1
                number_summary_dict[ScenarioDescriptionMapless.SUMMARY.NUM_MOVING_OBJECTS_EACH_TYPE][v["type"]] += 1
    ...
    def get_number_summary(scenario):
        ...
        # object
        number_summary_dict[ScenarioDescriptionMapless.SUMMARY.NUM_OBJECTS] = len(scenario[ScenarioDescriptionMapless.TRACKS])
        number_summary_dict[ScenarioDescriptionMapless.SUMMARY.OBJECT_TYPES] = \
            set(v["type"] for v in scenario[ScenarioDescriptionMapless.TRACKS].values())
        object_types_counter = defaultdict(int)
        for v in scenario[ScenarioDescriptionMapless.TRACKS].values():
            object_types_counter[v["type"]] += 1
        number_summary_dict[ScenarioDescriptionMapless.SUMMARY.NUM_OBJECTS_EACH_TYPE] = dict(object_types_counter)
        ...
        # moving object
        number_summary_dict.update(ScenarioDescriptionMapless._calculate_num_moving_objects(scenario))

        # Number of different dynamic object states
        # dynamic_object_states_types = set()
        # dynamic_object_states_counter = defaultdict(int)
        # for v in scenario[ScenarioDescriptionMapless.DYNAMIC_MAP_STATES].values():
        ...
        # number_summary_dict[ScenarioDescriptionMapless.SUMMARY.NUM_TRAFFIC_LIGHTS
        #                     ] = len(scenario[ScenarioDescriptionMapless.DYNAMIC_MAP_STATES])
        # number_summary_dict[ScenarioDescriptionMapless.SUMMARY.NUM_TRAFFIC_LIGHT_TYPES] = dynamic_object_states_types
        # number_summary_dict[ScenarioDescriptionMapless.SUMMARY.NUM_TRAFFIC_LIGHTS_EACH_STEP
        #                     ] = dict(dynamic_object_states_counter)

        # map
        # number_summary_dict[ScenarioDescriptionMapless.SUMMARY.NUM_MAP_FEATURES
        #                     ] = len(scenario[ScenarioDescriptionMapless.MAP_FEATURES])
        # number_summary_dict[ScenarioDescriptionMapless.SUMMARY.MAP_HEIGHT_DIFF
        #                     ] = ScenarioDescriptionMapless.map_height_diff(scenario[ScenarioDescriptionMapless.MAP_FEATURES])
        return number_summary_dict
    ...
    def sdc_moving_dist(scenario):
        ...
        """
        scenario = ScenarioDescriptionMapless(scenario)
    ...
    def num_object(scenario, object_type: Optional[str] = None):
        ...
        Returns:
            (int) The number of objects.
        """
        return ScenarioDescriptionMapless.get_num_objects(scenario, object_type)
    ...
    def num_moving_object(scenario, object_type=None):
        ...
        Returns:
            (int) The number of moving objects.
        """
        return ScenarioDescriptionMapless.get_num_moving_objects(scenario, object_type=object_type)
    ...
    def map_height_diff(map_features, target=10):
        ...
        for feature in map_features.values():
            if not MetaDriveType.is_road_line(feature[ScenarioDescriptionMapless.TYPE]):
                continue
            polyline = feature[ScenarioDescriptionMapless.POLYLINE]
            if len(polyline[0]) == 3:
